...
const ViewRouteStops = ({ selectedRouteId }) => {
  ...
  return (
    <div className="bg-neutral-800 p-4 text-text rounded-lg">
      ...
      <ul className="h-28 md:h-48 overflow-y-scroll space-y-2">
        {selectedRoute?.stopIds?.map((item, index) => {
          const stop = busStops.find((el) => el.id === item);

          return (
            <li key={item} className="flex items-center gap-1.5 relative">
              <i
                class={`${
              ...
              <span className="flex-1 px-2 py-2.5 bg-neutral-900/25 border border-neutral-100/20 rounded-lg text-neutral-100/80 whitespace-nowrap overflow-scroll">
                {stop.name}
              </span>
            </li>
          );
